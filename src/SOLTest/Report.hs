...
-- --------------------------------------------------------------------------------
-- CITE: A lot of the code in this file is heavliy inspired by code generated by AI
-- --------------------------------------------------------------------------------
...
groupByCategory definitions results = 
  ...
  let 
    -- make a map of definitions (key = test filename)
    defMap = Map.fromList [(tcdName definition, definition)| definition <- definitions]

    -- function for fold
    accFunc acc resName resReport = 
      -- looks up the test definition of current test result
...
-- |  Go through all the cat reports, look through each test report and 
...
addToHist hist cat =
  let
    ...
    -- compute rate
    rate = 
      if testCount == 0 -- idk how can this happen but tests show that it can
      then 0.0
      else fromIntegral passedTestCount / fromIntegral testCount 
  in
    -- add 1 to the bin specified by rate 
